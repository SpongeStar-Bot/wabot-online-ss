...
CFonts.say('\'wabot-online-ss\' By @SpongeStar-Bot', {
  font: 'console',
  align: 'center',
  gradient: ['red', 'magenta']
})
...
case 'stickerwa'://UPDATE MR.108P
                    ...
                    query = args.join(" ")
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/stickerwa?apikey=Ar-Mr108P&query=${query}`)
                    get_result = get_result.result[0].stickers
                    for (var x of get_result) {
                        ini_buffer = await getBuffer(`http://api.lolhuman.xyz/api/convert/towebp?apikey=Ar-Mr108P&img=${x}`)
...
case 'igvideo'://UPDATE MR.108P
				...
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
					 ige = body.slice(9)
                     anu = await fetchJson(`http://lolhuman.herokuapp.com/api/instagram?apikey=Ar-Mr108P&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
...
url = args[0]
url =`http://api.lolhuman.xyz/api/tiktok?apikey=Ar-Mr108P&url=https://vt.tiktok.com/ZSwWCk5o/${url}`
...
case 'ytmp3'://UPDATE MR105P
                    ...
                    ini_link = args[0]
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/ytaudio?apikey=Ar-Mr108P&url=${ini_link}`)
...
case 'ytmp4':
                case 'ytmp4':
                 if (!isPrem) return reply(ind.premium())
                    ini_link = args[0]
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/ytvideo?apikey=Ar-Mr108P&url=${ini_link}`)
                    get_result = get_result.result
...
case 'kusonime':
                    ini_url = args[0]
                    get_result = await fetchJson(`http://api.lolhuman.xyz/api/kusonime?apikey=Ar-Mr108P&url=${ini_url}`)
                    get_result = get_result.result
                    txt = `Title : ${get_result.title}\n`
...
case 'neko'://UPDATE MR.108P
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					to =  naru[Math.floor(Math.random() * naru.length)];
					nye = await getBuffer(to)
					baby.sendMessage(from, nye, image, { caption: 'kyaa one chan!!', quoted: mek })
					await limitAdd(sender)
					break
